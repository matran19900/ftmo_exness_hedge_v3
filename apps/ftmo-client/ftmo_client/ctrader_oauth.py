"""cTrader OAuth 2.0 helper (FTMO client vendor copy).

Vendor copy of ``server/app/services/ctrader_oauth.py``.

Per CEO assessment (Phase 4): the cTrader OAuth API is stable, no
anticipated updates Phase 5+. If a future bug fix or API change is
needed, update BOTH ``server/app/services/ctrader_oauth.py`` AND this
file. CPR foundation (step 4.4a): vendoring lets the FTMO client drop
its ``hedger_shared`` dependency for Phase 5 client-bundle work.

Module content otherwise identical to the server-side copy.
